...
\begin{center}
  ...
  {\large \textcolor{muted}{CompSpec — v0.3 \quad|\quad Companion to RDD v0.2 and MathFrame v0.1}}\\[0.6em]
    ...
    def action(theta):
        ...
        def potential_shift(s):
            ...
            gamma_s = jnp.einsum('k,ki,kc->ic', cos_s, cos_t, coeff[:,0,:]) \
                    + jnp.einsum('k,ki,kc->ic', sin_s, sin_t, coeff[:,0,:]) \
                    + jnp.einsum('k,ki,kc->ic', cos_s, sin_t, coeff[:,1,:]) \
                    - jnp.einsum('k,ki,kc->ic', sin_s, cos_t, coeff[:,1,:])
