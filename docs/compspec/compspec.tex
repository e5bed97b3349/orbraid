...
\begin{center}
  ...
  {\large \textcolor{muted}{CompSpec — v0.1 \quad|\quad Companion to RDD v0.1}}\\[0.6em]
...
\begin{enumerate}[leftmargin=1.8em]
  \item $R_3 \cdot \gamma(t) = R_3\,\gamma(t - T/3)$: imposes on mode $k$ that
        $R_3 \bfa_k = \bfa_k \cos(2\pi k/3) + \bfb_k \sin(2\pi k/3)$ and the
        analogous relation for $\bfb_k$.  Only modes $k \equiv 0 \pmod{3}$ survive
        in the $xy$-components; the $z$-components of modes $k \not\equiv 0
        \pmod 3$ are forced to zero.
...
\subsubsection{Implementation: constraint matrix $P_G$}

For each symmetry group $G$, we precompute a projection matrix $P_G$ such
that $\theta \in \Fix(G)$ iff $\theta = P_G \theta$.  This matrix is computed
once at initialisation via the Reynolds operator:
\[
  P_G = \frac{1}{|G|} \sum_{g \in G} \rho(g).
\]
All initialisations are drawn as $\theta = P_G z$ for $z \sim \mathcal{N}(0,
\sigma^2 I)$, guaranteeing they lie in $\Fix(G)$.  Gradient steps are
projected back via $\nabla \leftarrow P_G \nabla$ after each L-BFGS update.
